...
| Full-text index republished as a new GitHub Release (`fulltext-vN`) | `FULLTEXT_RELEASE_TAG` → `fulltext-vN` | `backend/Dockerfile` |
...
The full-text index follows the same republish discipline as the data caches — a new `fulltext-vN` release **and** bumping `FULLTEXT_RELEASE_TAG` in `backend/Dockerfile` in the same commit, or the deploy keeps fetching the old index. Unlike `data-vN`, it ships on its **own** tag with a single asset (`fulltext.sqlite.gz`), decoupled from the `data-vN` train — republishing the fulltext index never requires re-uploading the `data-vN` assets, and vice versa. It is optional at boot: an absent, unreadable, or version-mismatched file leaves `fulltextAvailable = false` and `/api/search` degrades silently, the same non-fatal pattern as `definitionsAvailable` and the citation graph.
